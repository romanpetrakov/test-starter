...
export const Profile = () => {
	...
	const {user, isLoading, getUserError, isLoadedUser} = useSelector((state) => state.auth);
   ...
   // const [isChanged, setIsChanged] = useState(false);
    ...
    return (
        <form method="POST" onSubmit={handleSubmit} autoComplete="off">
            <div className="mb-6">
                <Input placeholder='Имя' icon={'EditIcon'} type={'text'} onChange={handleChange}
                ref={inputRef}
            ...
            <div className="mb-6">
                <EmailInput isIcon={true} value={form?.email||""} onChange={handleChange} autocomplete="current-password"/>
            </div>
            <div className="mb-6">
                <PasswordInput value={form.password} onChange={handleChange} autoComplete="current-password"/>
            </div>
            {!isLoading &&
